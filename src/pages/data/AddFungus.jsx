...
function AddFungus() {
  ...
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("fungus")) || [];
  ...
  const validate = () => {
    ...
    if (!formData.FungusName.trim())
      err.FungusName = "Fungus Name is required";

    if (!formData.FungusHindiName.trim())
      err.FungusHindiName = "Hindi name is required";
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("fungus", JSON.stringify(updatedList));

    navigate("/data/fungus", {
    ...
    <div className="pt-20 ml-64 mb-20 min-h-screen">
      <div className="bg-white p-8 rounded-2xl shadow-lg max-w-xl mx-auto">
        <h1 className="text-2xl font-semibold text-center mb-6">
          {location.state?.editItem ? "Edit Fungus" : "Add Fungus"}
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Fungus Name */}
          <div>
            <label className="font-semibold">Fungus Name</label>
            <input
              type="text"
              name="FungusName"
              value={formData.FungusName}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-lg mt-1"
            />
            {errors.FungusName && (
              <p className="text-red-500 text-sm">{errors.FungusName}</p>
            )}
          </div>

          {/* Fungus Hindi Name */}
          <div>
            <label className="font-semibold">Fungus Hindi Name</label>
            <input
              type="text"
              name="FungusHindiName"
              value={formData.FungusHindiName}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-lg mt-1"
            />
            {errors.FungusHindiName && (
          ...
          <div className="flex justify-center gap-6 mt-5">
            ...
            >
              {location.state?.editItem ? "Update Fungus" : "Save Fungus"}
            </button>
          </div>
        </form>
      </div>
