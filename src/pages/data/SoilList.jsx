...
function SoilList() {
  ...
  // Load Data
  useEffect(() => {
    const saved = localStorage.getItem("soil");
    if (saved) {
  ...
  if (location.state?.newItem) {
    setProduct((prev) => {
      ...
      if (!exists) {
        ...
        localStorage.setItem("soil", JSON.stringify(updated));
        return updated;
      }
  ...
  const editproduct = (RequestId) => {
    ...
    navigate("/data/soil/addsoil", { state: { editItem: item } });
  };
  ...
  const tableData = product.map((item) => ({
    ...
    "Soil Name": item.SoilName,
    "Soil Hindi Name": item.SoilHindiName,
    "Type": item.Types,
    Action: (
  ...
  return (
    <div className="ml-64 bg-gray-100 min-h-screen">
      <div className="flex justify-between items-center mb-3">
        <h1 className="mt-5 text-2xl font-semibold">Soil List</h1>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => navigate("/data/soil/addsoil")}
          >
            + Add Herbicide
          </button>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-x-auto">
        {tableData.length > 0 ? (
          ...
          <p className="text-center text-gray-500 py-6">No soil added yet.</p>
        )}
      </div>
    </div>
  );
