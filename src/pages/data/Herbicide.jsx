...
function Herbicide() {
  ...
  // Load Data
  useEffect(() => {
    const saved = localStorage.getItem("herbicide");
    if (saved) {
  ...
  if (location.state?.newItem) {
    setProduct((prev) => {
      ...
      if (!exists) {
        ...
        localStorage.setItem("herbicide", JSON.stringify(updated));
        return updated;
      }
  ...
  const editproduct = (RequestId) => {
    ...
    navigate("/data/herbicide/addherbicide", { state: { editItem: item } });
  };
  ...
  const tableData = product.map((item) => ({
    ...
    "Herbicide Name": item.HerbicideName,
    "Herbicide Hindi Name": item.HerbicideHindiName,
    "Type": item.Types,
    Action: (
  ...
  return (
    <div className="ml-64  min-h-screen">
      <div className="flex justify-between items-center mb-3">
        <h1 className="mt-5 text-2xl font-semibold">Herbicide List</h1>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => navigate("/data/herbicide/addherbicide")}
          >
            + Add Herbicide
          </button>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-md overflow-x-auto">
        {tableData.length > 0 ? (
          ...
          <p className="text-center text-gray-500 py-6">No herbicide added yet.</p>
        )}
      </div>
    </div>
  );
