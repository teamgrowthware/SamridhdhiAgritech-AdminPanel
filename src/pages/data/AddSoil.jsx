...
function AddSoil() {
  ...
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("soil")) || [];
  ...
  const validate = () => {
    ...
    if (!formData.SoilName.trim())
      err.SoilName = "Soil Name is required";

    if (!formData.SoilHindiName.trim())
      err.SoilHindiName = "Hindi name is required";
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("soil", JSON.stringify(updatedList));
    ...
    <div className="pt-20 ml-64 mb-20 min-h-screen">
      <div className="bg-white p-8 rounded-2xl shadow-lg max-w-xl mx-auto">
        <h1 className="text-2xl font-semibold text-center mb-6">
          {location.state?.editItem ? "Edit Soil" : "Add Soil"}
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Fungus Name */}
          <div>
            <label className="font-semibold">Soil Name</label>
            <input
              type="text"
              name="SoilName"
            ...
            />
            {errors.SoilName && (
              <p className="text-red-500 text-sm">{errors.SoilName}</p>
            )}
          </div>

          {/* Fungus Hindi Name */}
          <div>
            <label className="font-semibold">Soil Hindi Name</label>
            <input
              type="text"
              name="SoilHindiName"
              value={formData.SoilHindiName}
              onChange={handleChange}
              className="w-full px-3 py-2 border rounded-lg mt-1"
            />
            {errors.SoilHindiName && (
