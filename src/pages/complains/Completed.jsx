...
function Complited() {
  ...
  // Load Data
  useEffect(() => {
    const saved = localStorage.getItem("complite");

    if (saved) {
      setProduct(JSON.parse(saved));
    } else {
      localStorage.setItem("complite", JSON.stringify(defaultData));
    ...
    if (location.state?.newItem) {
      setProduct((prev) => {
        ...
        if (!exists) {
          ...
          localStorage.setItem("complite", JSON.stringify(updated));
          return updated;
        }
        return prev;
      });
  ...
  return (
    <div className="ml-64  min-h-screen">
      <div className="flex justify-between items-center mb-3">
        <h1 className="mt-5 text-2xl font-semibold">Completed Complaints</h1>
