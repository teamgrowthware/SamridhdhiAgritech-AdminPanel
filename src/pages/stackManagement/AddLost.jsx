...
function AddLost() {
  ...
  const handleSubmit = (e) => {
    ...
    const storedStock = JSON.parse(localStorage.getItem("loststock")) || [];
    const nextId =
      storedStock.length > 0 ? storedStock[storedStock.length - 1].id + 1 : 1;

    const newStock = { id: nextId, ...formData };
    localStorage.setItem("loststock", JSON.stringify([...storedStock, newStock]));
