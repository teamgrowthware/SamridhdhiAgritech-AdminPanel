...
function Adddamaged() {
  ...
  const handleSubmit = (e) => {
    ...
    const storedStock = JSON.parse(localStorage.getItem("damagedstock")) || [];
    const nextId =
      storedStock.length > 0 ? storedStock[storedStock.length - 1].id + 1 : 1;

    const newStock = { id: nextId, ...formData };
    localStorage.setItem("damagedstock", JSON.stringify([...storedStock, newStock]));

    toast.success("New Stock added successfully!");
    navigate("/stack/damaged");
  };
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg max-w-lg mx-auto relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-center gap-6 mt-6">
            <button
              type="button"
              onClick={() => navigate("/stack/damaged")}
